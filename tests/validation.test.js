...
function validateDateRange(startDate, endDate) {
    if (!validateDateFormat(startDate)) {
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Invalid startDate format. Expected YYYY-MM-DD, got: ${startDate}`);
    }
    if (!validateDateFormat(endDate)) {
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, `Invalid endDate format. Expected YYYY-MM-DD, got: ${endDate}`);
    }
    if (new Date(startDate) > new Date(endDate)) {
        throw new types_js_1.McpError(types_js_1.ErrorCode.InvalidParams, "startDate cannot be after endDate");
    }
}
describe("Date Validation", () => {
    ...
    describe("validateDateRange", () => {
        ...
        it("should throw McpError for invalid start date", () => {
            expect(() => validateDateRange("invalid-date", "2024-01-31")).toThrow(types_js_1.McpError);
        });
        it("should throw McpError for invalid end date", () => {
            expect(() => validateDateRange("2024-01-01", "invalid-date")).toThrow(types_js_1.McpError);
        });
        it("should throw McpError when start date is after end date", () => {
            expect(() => validateDateRange("2024-01-31", "2024-01-01")).toThrow(types_js_1.McpError);
        });
    });
});
describe("Environment Validation", () => {
    ...
    it("should throw when required environment variables are missing", () => {
        ...
        expect(() => validateEnvironment()).toThrow("Missing required environment variables: GOOGLE_CLIENT_EMAIL, GOOGLE_PRIVATE_KEY, GA_PROPERTY_ID");
    });
    it("should throw when only some environment variables are missing", () => {
        process.env.GOOGLE_CLIENT_EMAIL = "test@example.com";
        delete process.env.GOOGLE_PRIVATE_KEY;
        delete process.env.GA_PROPERTY_ID;
        expect(() => validateEnvironment()).toThrow("Missing required environment variables: GOOGLE_PRIVATE_KEY, GA_PROPERTY_ID");
    });
});
